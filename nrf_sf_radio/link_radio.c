...
#define NRF_SF_RADIO_TX_CHAIN_DELAY_TICKS (160U)
                              ...
                              uint32_t rx_end_ticks)
{
    ...
    nrf_sf_radio_wait_until_abs(&NRF_RADIO->EVENTS_ADDRESS,
                                rx_window_end_ticks);
    //printf("ready to address %ld \n", nrf_sf_radio_get_last_address_time_ticks() - nrf_sf_radio_get_last_ready_time_ticks());
    if (NRF_RADIO->EVENTS_ADDRESS != 0) {
        nrf_sf_radio_wait_until_abs(&NRF_RADIO->EVENTS_END,
                                    rx_end_ticks +
                                        NRF_SF_RADIO_TX_CHAIN_DELAY_TICKS);
